...
methods_combination <- read.csv("features/04_imposters_testing_refine_features.csv", stringsAsFactors = F)
...
for(method in 1:length(methods_combination$n_best_imposters)){
  ...
  cat("\n\nConfiguration:", methods_combination$n_best_imposters[method], "imposters", methods_combination$MFW[method], "MFW", methods_combination$distance[method], "-- imposters group", methods_combination$imposters_group[method], "\n\n", file = filename, append = T)
  
  cat("Imposters:", validation_imposters[[methods_combination$imposters_group[method]]], "\n\n", sep = "\n", file = filename, append = T)
  
  imposters_selection <- which(all_imposters %in% validation_imposters[[methods_combination$imposters_group[method]]])
